...
oldtimer dd 0
	...
	crossed dw 0
	width dw 8
	restartt:db 'press R to restart or S to finish',0
...
game_over_msg: db'          ', 0 ;'GAME OVER!', 0
...
background_loopoi:
    ...
    stosw
    loop background_loopoi

    ; Draw top border (underscore line)
    mov al, '_'
    mov ah, 0x4E      ; Red background, yellow text
    mov di, 0
    mov cx, 80
top_border:
    stosw
    ...
    ; Draw bottom border (underscore line)
    mov di, 3680
    mov cx, 80
bottom_border:
    stosw
...
kbsir:
    ...
    mov word [stay], 1
    jmp endd
	nextcomr: cmp al,0x13
	jne nextcomrs
	mov word[restart],1
	jmp endd
nextcomrs:cmp al,0x1F
	jne nextcom
	mov word[restart],2
	jmp endd
nextcom:
...
nextLetterstartE: 
	...
	pusha
	 mov word[restart] , 0
     mov  word[rect_pos1] , 80   
      mov word[rect_pos2] , 120   
      mov word[rect_pos3 ],90     
     mov byte[ Birddirection] , 'd'  
    ; mov [BUFFER_SIZE] equ 4000 
	 mov  word[rect_1u] , 10
	 mov  word[rect_1d] , 9
	  mov word[rect_2u] , 10
	  mov word[rect_2d ],9
...
pusha
		...
		shl ax, 1 		; turn into byte offset
		mov di, ax 		; point di to required location
		mov ah, 0x22		   ; load attribute in ah
		mov cx,54
...
mov al, 0x2D 		; ASCII of '-'
		...
		add di, 2 		; move to next screen location 
		call sleepp;
		loop topLinei		; repeat the operation cx times

		mov cx, 10

rightLinei:	mov al, 0x7c 		; ASCII of '|'
		mov [es:di], ax 	; show this char on screen
		add di, 160 		; move to next screen location 		
		call sleepp;
		loop rightLinei		; repeat the operation cx times
		
		mov cx, 54

bottomLinei:	mov al, 0x2D 		; ASCII of '-'
		mov [es:di], ax 	; show this char on screen
		sub di, 2 		; move to next screen location 
		call sleepp;
		loop bottomLinei		; repeat the operation cx times

		mov cx, 10
		;sub cx, 2

leftLinei:	mov al, 0x7c 		; ASCII of '|'
		mov [es:di], ax 	; show this char on screen
		sub di, 160 		; move to next screen location 		
		call sleepp;
		loop leftLinei	; repeat the operation cx times
